...
;General
  ...
  InstallDir "$PROGRAMFILES\image-to-array"
  
  ;Get installation folder from registry if available
  InstallDirRegKey HKCU "Software\image-to-array" ""

  ;Request application privileges for Windows Vista
  RequestExecutionLevel admin
  
  !define REG_UNINSTALL "Software\Microsoft\Windows\CurrentVersion\Uninstall\image-to-array"
...
SectionIn RO
  ...
  File ${PROJECT_BUILD_DIR}\ita.exe
  File /nonfatal ${PROJECT_BUILD_DIR}\*.dll
  File /nonfatal ${PROJECT_BUILD_DIR}\natives\native\win\64\*.dll

  ;Store installation folder
  WriteRegStr HKCU "Software\image-to-array" "" $INSTDIR
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\ita.exe"
  Delete "$INSTDIR\*.dll"
  RMDir /r "$INSTDIR"

  !insertmacro un.RemoveFromSystemPath
  
  DeleteRegKey /ifempty HKCU "Software\image-to-array"
